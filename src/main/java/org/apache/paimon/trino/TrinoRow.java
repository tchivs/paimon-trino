...
public class TrinoRow implements InternalRow, Serializable {
    ...
    private abstract static class AbstractTrinoArray implements InternalArray {
        ...
        public float getFloat(int pos) {
            return Float.intBitsToFloat(
                    Math.toIntExact(
                            (long) TypeUtils.readNativeValue(REAL, block, getPosition(pos))));
        }
        ...
        public Timestamp getTimestamp(int pos, int precision) {
            long value =
                    (long) TypeUtils.readNativeValue(TIMESTAMP_MICROS, block, getPosition(pos));
            return Timestamp.fromMicros(value);
        }
    ...
    private static class TrinoArray extends AbstractTrinoArray {
    ...
    private static class TrinoMap implements InternalMap {
        private final SqlMap sqlMap;

        TrinoMap(SqlMap sqlMap) {
            this.sqlMap = sqlMap;
        }
    ...
    private static class TrinoNestedRow implements InternalRow {
        ...
        public float getFloat(int pos) {
            Block fieldBlock = rowBlock.getFieldBlock(pos);
            return Float.intBitsToFloat(
                    Math.toIntExact((long) TypeUtils.readNativeValue(REAL, fieldBlock, position)));
        }
        ...
        public Decimal getDecimal(int pos, int precision, int scale) {
            ...
            Object value =
                    TypeUtils.readNativeValue(
                            DecimalType.createDecimalType(precision, scale), fieldBlock, position);
            if (precision <= MAX_SHORT_PRECISION) {
                return Decimal.fromUnscaledLong((Long) value, precision, scale);
            } else {
                long high = ((Int128) value).getHigh();
    ...
    /**
     * TrinoArrayView implementation with offset and length for viewing part of a Block. Used for
     * Map key/value arrays.
     */
    private static class TrinoArrayView extends AbstractTrinoArray {
